...
public class PerfectSquares {
    ...
    public int numSquares(int n) {
        int[] leastSquaresForIndex = new int[n + 1];
        for (int i = 0; i < n; i++)
            leastSquaresForIndex[i] = Integer.MAX_VALUE;

        for (int i = n; i >= 0; i--) {
            for (int square : SQUARES) {
                int previousIndex = i + square;
                if ((previousIndex < n + 1) && (leastSquaresForIndex[previousIndex] + 1 < leastSquaresForIndex[i])) {
                    leastSquaresForIndex[i] = leastSquaresForIndex[previousIndex] + 1;
                }
            }
        }
        return leastSquaresForIndex[0];
    }
}
